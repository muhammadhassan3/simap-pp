                            ...
                            <tbody id="proposalTableBody">
                                @foreach ($proposal as $index => $p)
                                    <tr class="hover:bg-[#FFFFEC]">
                                        ...
                                        <td class="px-4 py-2 text-center">{{ $p->tempat_proyek->nama_tempat ?? '-' }}
                                        </td>
                                        <td class="px-4 py-2 text-center">
                                            {{ \Carbon\Carbon::parse($p->tanggal_pengajuan)->format('d/m/Y') }}</td>
                                        <td class="px-4 py-2 text-center">
                                            {{ 'Rp. ' . number_format($p->harga_proyek, 0, ',', '.') }}</td>
                                        <td class="px-4 py-2 text-center">
                                        ...
                                        <td class="px-4 py-2 text-center">
                                            @if ($p->status_proposal === 'pending')
                                                <form
                                                    action="{{ route('proposal.updateStatus', ['id_pengajuan_proposal' => $p->id_pengajuan_proposal, 'status' => 'disetujui']) }}"
                                                ...
                                                <form
                                                    action="{{ route('proposal.updateStatus', ['id_pengajuan_proposal' => $p->id_pengajuan_proposal, 'status' => 'ditolak']) }}"
                                                    method="POST" class="d-inline">
                                                    @csrf
                                                    @method('PUT')
                                        ...
                                        <td class="px-4 py-2 text-center">
                                            <a href="{{ route('pengajuan_proposal.edit', $p->id_pengajuan_proposal) }}"
