        ...
        <div class="card-body">
            <div class="d-flex align-items-center mb-3">
                <a href="{{ url()->previous() }}" class="text-dark fw-semibold text-decoration-none d-flex align-items-center gap-2">
                    <i class="bi bi-arrow-left fs-4"></i>Edit Jadwal
                </a>
            </div>
            ...
            <form id="edit-form" action="/penjadwalan_proyek/update/{{ $jadwal->id }}" method="POST">
                ...
                <input type="hidden" id="tanggal_mulai_proyek"
                    value="{{ $jadwal->proyekDisetujui->tanggal_mulai ?? '' }}">
                <input type="hidden" id="tanggal_selesai_proyek"
                    value="{{ $jadwal->proyekDisetujui->tanggal_selesai ?? '' }}">
                <input type="hidden" name="id_proyek_disetujui" value="{{ $jadwal->id_proyek_disetujui }}">

                <div class="mb-3">
                    <label class="form-label">Nama Proyek</label>
                    <input type="text" class="form-control"
                        value="{{ $jadwal->proyekDisetujui->pengajuanProposal->nama_proyek ?? 'Tidak Diketahui' }}"
                        disabled>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control"
                        value="{{ $timProyek ? $timProyek->pekerja->nama : 'Tidak Tersedia' }}" disabled>
                </div>

                <div class="mb-3">
                    <label class="form-label">No HP Supervisor</label>
                    <input type="text" class="form-control"
                        value="{{ $timProyek ? $timProyek->pekerja->no_hp : 'No HP Tidak Ditemukan' }}" disabled>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" name="status" required>
                        ...
                        <option value="sedang dikerjakan"
                            {{ $jadwal->status == 'sedang dikerjakan' ? 'selected' : '' }}>Sedang Dikerjakan</option>
                ...
                <button type="button" class="btn btn-warning w-100" onclick="konfirmasiEdit()">Simpan
                    Perubahan</button>
            </form>
        </div>
    </div>
